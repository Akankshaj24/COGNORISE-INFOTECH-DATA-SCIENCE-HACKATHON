# -*- coding: utf-8 -*-
"""
Created on Wed Apr 24 19:57:24 2024
...
y_pred_test = best_model.predict(df_encoded_test)+1


# Create a DataFrame to store results
results_df = pd.DataFrame(y_pred_test,columns=["Group_no"])

# Remove existing index (if any) and create a new numerical index
results_df.index = pd.RangeIndex(start=1, stop=len(results_df) + 1, name='serial number')
print(results_df)

# Optionally, add additional results from cv_results_
# cv_results_ = grid_search.cv_results_
# results_df = pd.DataFrame(cv_results_)  # This will include all combinations

# Save the DataFrame to a CSV file
results_df.to_csv('C&T Bank_sample_submission.csv')